...
def place_coindcx_futures_order(pair, side, price, sl, tp):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            err_msg = f"⚠️ *CoinDCX Order Failed!*\nPair: {pair}\nError: {res_data.get('message', res_data)}"
...
async def run_auto_trade_engine():
    ...
    while True:
        try:
            for symbol in SYMBOLS:
                ...
                bull_cross = (
                    (pre_prev["close"] <= pre_prev["High_20"]) and 
                    (prev["close"] > prev["High_20"]) and 
                    (prev["close"] > prev["EMA_50"]) and 
                    (prev["volume"] > prev["Vol_MA"])
                )

                bear_cross = (
                    (pre_prev["close"] >= pre_prev["Low_20"]) and 
...
async def lifespan(app: FastAPI):
    asyncio.create_task(run_auto_trade_engine())
    yield
...
async def health_check():
    return {"status": "Bot is running and scanning Binance/CoinDCX 24/7"}
